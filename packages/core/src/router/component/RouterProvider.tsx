...
import {useDi, useFromDi} from '../../index';
...
const RouterProvider: React.FunctionComponent<{}> = (props) => {
  ...
  useEffect(() => {
    const hydrateRoute = () => {
      ...
      router.resolve({pathname: payload.pathname, query: payload.query, url: window.location.href})
    }
  ...
  return (
    <BindToDi services={(container) => {
      container.bind<RouterServiceType>(serviceIds.routerService).toConstantValue({
        ...
        linkToHome(): string {
          throw new Error('linkToHome() is deprecated, since the home-route is based on your configuration. Please use linkTo(\'name-of-your-home-route\') instead.');
        },
        navigate(url: string) {
          history.push(url)
        },
        navigateToHome() {
          throw new Error('linkToHome() is deprecated, since the home-route is based on your configuration. Please use navigateTo(\'name-of-your-home-route\') instead.');
        },
